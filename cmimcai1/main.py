...
class secondBiggest:
    ...
    def move(self, hand, others, card, scores):
        # You should write your code that moves every turn here
        biggestCards = sorted(hand, reverse=True)
        other_second_biggest_cards = []
        for other in others:
          other_sorted = sorted(other, reverse=True)
          other_second_biggest_cards.append(other_sorted[1])
        return other_second_biggest_cards[0]
      
#if card is higher than highest card of both of opponents, then play the card that is 1 above their highest (for later rounds)
#if not, play value of card - 1
...
class arhan_bot:
    ...
    def move(self, hand, others, card, scores):
      ...
      for other in others:
        ...
        other_biggest_cards.append(other_sorted[0])
      if (len(hand) < 10):
        if card > other_biggest_cards[0] and card > other_biggest_cards[1]:
          return (max(other_biggest_cards[0], other_biggest_cards[1]))+1
      else:
        return (card-1)
...
LOCAL_STRATS = [new_bot(), random_bot(), random_bot()]
